...
import { fieldString } from './helper'
//! 去掉resolve 也可以得到 rest api 路由, 但是不能查询, 所以还是要带上
const schema = makeExecutableSchema({ typeDefs , resolvers   })

//* rest api 与 graphql api 是一致的, 为保持这种一致性及使用方便, 
//* graphql 得受到一定得限制, 例如: 函数得输入参数最好基本类型.
//* 也不要有层级得结构. 函数参数与变量参数一致, 函数参数与变量参数, 也可以没有.
//* 返回值可以是层级结构, 
...
// TODO: 8. 查询错误的情况
// TODO: 9. 查询返回字段的情况 无需实现, 只要 graphql schema 定义就可以
// TODO: 10. post 提交情况 在 getVariable() 判断 get 还是 post 请求
// TODO: 11. 权限处理
// TODO: 12. 错误处理
    ...
    for (let i = 0; i < definitions.length; i++) {
        ...
        if (definitions[i].name && operators.includes(definitions[i].name.value)) {
            ...
            fields.forEach(field => {
                ...
                //! 还可以利用 请求对象 ctx.query 来确定返回的参数, 不过, 在 graphql schema 里定义也是一样的
                //! 返回的是 schema 里定义了的字段, 这里就先不实现这个功能了
                // console.log("路由路径---->  ", path, fnBody)
                return router.all(path, async (ctx ) => {
                    let response
                    try {
                        // 发送 graphql 请求, 问 graphql 要数据
                        // TODO: 是不是和 resolver和graphql 是关联的? 
                        // TODO: 如果字段上有权限指令, 是 报错 还是 取不到 还是 取得到?
                        // TODO: 有待测试
