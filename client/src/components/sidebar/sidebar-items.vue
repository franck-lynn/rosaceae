<template>
    <ul :class="['sidebar-list-a', 'sidebar-list',isDeepOne ? 'nav-list' : 'sub-list', {'narrow-to-region': isNarrow}]">
        <li v-for="(item, index) in items" :key="index" :class="[isDeepOne? 'nav-items': 'nav-sub-items',
          ...
          {'nav-sub-items-show': item.children  && currentTarget === item.title}]" @click="handleClick($event, item.title)">
...
<script>
    ...
    export default defineComponent({
        ...
        setup(props, ctx) {
            ...
            const sidebarName = 'sidebar-list-a'
            const offset = 10
            ...
            const handleClick = (e, itemTitle) => {
                // 一级菜单在窄状态下除了没有下级菜单外总是显示, 若都显示的话, 可在 component标签加 'title-or-link' 类, findParentNode() 找这个类
                currentTarget.value = itemTitle // 在 li 上设置 当前点击类, 实现点击时显示菜单

                const el = findParentNode(e.target, 'nav-list-title')
                const clickedDeep = el ? parseInt(el.lastElementChild.className.match(/\d/g)['0']) : null

                if (props.deep === 1 && clickedDeep === 1) {
                    // 判断点击的是不是第1级
                    ctx.emit('isActived', { deep: props.deep, itemTitle })
                } else {
                    if (!el) { // 如果el 不存在, 就说明是最后一级菜单
                        if (props.isNarrow) {
                            ctx.emit('isActived', { el, itemTitle }) // 发射这个事件
                        }
                    }
                    // 总是要找到 el的父组件
                    if (el && el.parentNode.className.includes('nav-sub-items-show') && props.deep === clickedDeep) {
            ...
            const cancleMenu = e => {
                // 点击其他地方关闭下拉菜单
                const el = findParentNode(e.target, sidebarName)
                if (props.isNarrow) {
                    if (!el || !el.className || !el.className.includes(sidebarName)) {
